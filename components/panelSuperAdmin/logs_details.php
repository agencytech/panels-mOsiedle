...
$sql = "SELECT logs.id, users.fullName, logs.when, logs.object_id, logs.object_type, log_types.type,logs.description, logs.before, logs.after FROM `logs` join users on users.id=logs.user_id join log_types on log_types.id=logs.type where logs.id='$id';";
        ...
        <div class="flex flex-col md:flex-row md:space-x-4">
                ...
                <div class="flex flex-col md:flex-row gap-2 md:mr-2">
                    <div class="flex flex-col w-full md:w-1/2">
                        <div class="flex flex-col">
                            ...
                            <input type="text" name="user_id" id="user_id" value="<?php echo $row['fullName'];?>" class="capitalize text-sm text-gray-900 border border-gray-400 rounded-full px-4 py-2 focus:outline-none focus:border-indigo-500 transition-all duration-500" disabled>
                        </div>
                    </div>
                    <div class="flex flex-col w-full md:w-1/2">
                        <div class="flex flex-col">
                            <label for="when" class="pb-2 pt-2 font-medium text-xs text-[#5d5d5d]">Kiedy</label>
                            <input type="text" name="when" id="when" value="<?php echo $when; ?>" class="text-sm border border-gray-400 text-gray-900 rounded-full px-4 py-2 focus:outline-none focus:border-indigo-500 transition-all duration-500" disabled>
                        </div>
                    </div>
                </div>
                <div class="flex flex-col">
                    <label for="object_id" class="pb-2 font-medium pt-2 text-xs text-[#5d5d5d]">Id obiektu</label>
                    <input type="text" name="object_id" id="object_id" value="<?php echo $objectId; ?>" class="text-sm border border-gray-400 text-gray-900 rounded-full px-4 py-2 focus:outline-none focus:border-indigo-500 transition-all duration-500" disabled>
                </div>
                <div class="flex flex-col">
                    <label for="object_type" class="pb-2 pt-2 font-medium text-xs text-[#5d5d5d]">Typ obiektu</label>
                    <input type="text" name="object_type" id="object_type" value="<?php echo $objectType; ?>" class="text-sm border border-gray-400 text-gray-900 rounded-full px-4 py-2 focus:outline-none focus:border-indigo-500 transition-all
            ...
            <div class="flex flex-col w-full md:w-1/2">
                <div class="flex flex-col">
                    ...
                    if ($objectType == "users") {$sql = "SELECT fullName FROM `users` where id='$objectId';"; 
                        $result = mysqli_query($conn, $sql);
                        $row = mysqli_fetch_assoc($result);
                        echo $row['fullName'];
                    ...
                    ?>" class="border border-gray-400 rounded-full text-gray-900 px-4 py-2 focus:outline-none focus:border-indigo-500 transition-all duration-500 text-sm" disabled>
                </div>
                <div class="flex flex-col">
                    ...
                    border border-gray-400 font-semibold capitalize rounded-full px-4 py-2 focus:outline-none focus:border-indigo-500 transition-all duration-500 text-sm" disabled>
                </div>
                <div class="flex flex-col">
                    <label for="description" class="pb-2 text-xs pt-2 font-medium text-[#5d5d5d]">Opis</label>
                    <textarea name="description" id="description" cols="30" rows="1" class="text-sm border border-gray-400 text-gray-900 rounded-full px-4 py-2 focus:outline-none focus:border-indigo-500 transition-all duration-500" disabled><?php echo $description; ?></textarea>
                </div>
                <div class="flex flex-col h-full">
